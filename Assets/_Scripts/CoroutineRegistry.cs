...
public static class CoroutineRegistry
{
    ...
    public static Coroutine RunAndTrack(MonoBehaviour owner, IEnumerator routine, bool shouldBlockGameplay)
    {
        ...
        return coroutine; //returning the coroutine is not stricly necessary, but can potentially be helpful for checks in the future
    }



    private static IEnumerator ExecuteAndTrackCoroutine(MonoBehaviour owner, IEnumerator routine, bool shouldBlockGameplay = false)
    {
        ...
        activeCoroutines.Add(new TrackedCoroutine(owner, thisCoroutine, shouldBlockGameplay));
    ...
    /// <summary>
    /// Returns true if any coroutines are currently preventing the gameplay from continuing (delaying next action/turn)
    /// </summary>
    /// <returns></returns>
    public static bool CheckIsGameplayBlocked()
    {

        foreach( TrackedCoroutine tr in activeCoroutines)
        {
            if (tr.ShouldBlockGameplay)
            {
                return true;
            }
        }
